			...
			if (KitAPI.getKit(p) == "Nenhum") {
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
					...
					public void run() {
						...
						p.sendMessage("§4§lKIT§F§LPVP §a: §7§lVoce entrou no Parkour!");
						BarAPI.setMessage(p, "§4§lKIT§F§LPVP §a: §7§lVoce entrou no §c§lParkour", 3);
				...
				if (args[0].equalsIgnoreCase("set")) {
				    if (p.hasPermission("fly.set")) {
				    	Set(p, "Parkour");
				    	p.sendMessage("§4§lKIT§F§LPVP §a: §b§lVoce setou o Parkour!");
				}
			  }
			}
			return false;
